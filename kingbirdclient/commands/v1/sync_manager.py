...
class ResourceSync(base.KingbirdLister):
    ...
    def get_parser(self, parsed_args):
        ...
        parser.add_argument(
            '--force',
            action='store_true',
            help='Forcefully sync the resources to target_regions'
        )
    ...
    def _get_resources(self, parsed_args):
        ...
        if parsed_args.resources:
            ...
            error_msg = "You need to specify the resources to be synced."
            raise exceptions.KingbirdClientException(error_msg)

        if parsed_args.source:
            kwargs['source'] = parsed_args.source
        else:
            error_msg = "You need to specify source region" \
                        " from which resources have to be synced."
            raise exceptions.KingbirdClientException(error_msg)

        if parsed_args.target:
            kwargs['target'] = parsed_args.target
        else:
            error_msg = "You need to mention the target region" \
                        " to which resources have to be synced."
            raise exceptions.KingbirdClientException(error_msg)
